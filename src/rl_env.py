...
class CreditLimitEnv:
    def __init__(self, df, provision_bins):
        ...
        self.df['UR'] = pd.cut(self.df['UR_456'], bins=[-np.inf, 0.25, 0.5, 0.75, 1.0, np.inf],
                               labels=False, include_lowest=True)
        self.df['PR'] = pd.cut(self.df['PR_456'], bins=[-np.inf, 0.25, 0.5, 0.75, 1.0, np.inf],
                               labels=False, include_lowest=True)
    ...
    def step(self, action):
        ...
        if os.environ.get("DEBUG", "0") == "1":
            ...
            print(f"Delta Prov Bin: {new_dp_bin}, GT Balance: {info['actual_balance']:.2f}")
